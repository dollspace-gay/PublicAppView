...
export function OspreyStatus() {
  const { data, isLoading, isError, error, refetch } = useQuery<OspreyStatusResponse>({
    ...
    return (
      <Card data-testid="card-osprey-status">
        <CardHeader className="pb-3">
          ...
          <CardDescription>Advanced moderation and spam detection</CardDescription>
        </CardHeader>
      </Card>
    );
  }
  ...
  if (hasError) {
    const errorMessage = data?.error || (error instanceof Error ? error.message : "Network request failed");
      ...
      <Card data-testid="card-osprey-status">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="destructive" data-testid="badge-osprey-error">Error</Badge>
          </div>
          <CardDescription>Advanced moderation and spam detection</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="p-3 bg-destructive/10 rounded text-sm text-destructive" data-testid="text-status-error">
      ...
      <Card data-testid="card-osprey-status">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="secondary" data-testid="badge-osprey-disabled">Disabled</Badge>
          </div>
          <CardDescription>Advanced moderation and spam detection</CardDescription>
        </CardHeader>
        <CardContent className="space-y-2">
          ...
          <ol className="text-sm text-muted-foreground space-y-1 list-decimal list-inside">
            <li>Clone Osprey into <code className="bg-muted px-1 rounded">osprey/</code> directory</li>
            <li>Run <code className="bg-muted px-1 rounded">./scripts/enable-osprey.sh</code></li>
          </ol>
        </CardContent>
      </Card>
    );
    ...
    <Card data-testid="card-osprey-status">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <StatusIcon className={`h-4 w-4 ${statusColor}`} data-testid="icon-osprey-health" />
            <Badge 
        ...
        <CardDescription>Real-time moderation pipeline with label application</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {data.bridge && (
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-sm font-medium">
              <Activity className={`h-4 w-4 ${bridgeHealthy ? 'text-green-500' : 'text-red-500'}`} />
              <span>Firehose Bridge</span>
              <Badge variant={bridgeHealthy ? "outline" : "destructive"} className="ml-auto">
            ...
            {data.bridge.kafka && (
              <div className="flex items-center justify-between text-sm pl-6">
                ...
                <Badge variant={data.bridge.kafka.connected ? "outline" : "destructive"}>
        ...
        {data.effector && (
          <div className="space-y-2 pt-2 border-t">
            <div className="flex items-center gap-2 text-sm font-medium">
              <Shield className={`h-4 w-4 ${effectorHealthy ? 'text-green-500' : 'text-red-500'}`} />
              <span>Label Effector</span>
              <Badge variant={effectorHealthy ? "outline" : "destructive"} className="ml-auto">
            ...
            {data.effector.kafka && (
              <div className="flex items-center justify-between text-sm pl-6">
                <span className="text-muted-foreground">Kafka:</span>
                <Badge variant={data.effector.kafka.connected ? "outline" : "destructive"}>
            ...
            {data.effector.labels && (
              <>
                <div className="flex items-center justify-between text-sm pl-6">
                  ...
                  <span className="font-mono text-green-600" data-testid="text-labels-applied">
                    {data.effector.labels.applied.toLocaleString()}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm pl-6">
                  <span className="text-muted-foreground">Labels Negated:</span>
                  <span className="font-mono text-amber-600" data-testid="text-labels-negated">
