...
async function runBenchmarks() {
  ...
  const enhancedResult = await benchmark(
    'Enhanced Hydrator (baseline)',
  ...
  console.log('Method'.padEnd(40) + 'Duration'.padEnd(12) + 'Batches'.padEnd(10) + 'Cache Stats');
  console.log('━'.repeat(80));
  
  results.forEach(result => {
    const cacheStats = result.cacheHits !== undefined 
      ? `${result.cacheHits}/${result.cacheMisses} (hits/misses)`
      : 'N/A';
    const batches = result.dataLoaderBatches !== undefined 
  ...
  const dataLoaderImprovement = ((enhancedResult.duration - dataLoaderColdResult.duration) / enhancedResult.duration * 100).toFixed(1);
  const warmCacheImprovement = ((dataLoaderColdResult.duration - dataLoaderWarmResult.duration) / dataLoaderColdResult.duration * 100).toFixed(1);
  const overlapBenefit = ((dataLoaderColdResult.duration - dataLoaderOverlapResult.duration) / dataLoaderColdResult.duration * 100).toFixed(1);
  ...
  console.log(`  • DataLoader batches: ${dataLoaderColdResult.dataLoaderBatches} parallel queries (vs potentially ${postUris.length * 5}+ sequential)`);
  ...
  if (threadPost.length > 0) {
    ...
    const threadResult = await benchmark(
      'DataLoader Thread Hydration',
      () => dataLoaderHydrator.hydratePosts(threadUris, viewerDid, dataLoader3)
    );
    
    console.log(`Thread hydration (${threadUris.length} posts): ${threadResult.duration.toFixed(2)}ms with ${threadResult.dataLoaderBatches} batches`);
  }
...
// Run benchmarks
runBenchmarks().catch(console.error).finally(() => process.exit(0));