...
async function runBenchmarks() {
  ...
  const coldImprovement = ((enhancedResult.duration - optimizedColdResult.duration) / enhancedResult.duration * 100).toFixed(1);
  const warmImprovement = ((enhancedResult.duration - optimizedWarmResult.duration) / enhancedResult.duration * 100).toFixed(1);
  
  console.log('\n📈 Performance Improvements:');
  console.log(`  • Cold cache: ${coldImprovement}% faster`);
  console.log(`  • Warm cache: ${warmImprovement}% faster`);
  console.log(`  • Cache hit rate: ${(optimizedWarmResult.cacheHits! / (optimizedWarmResult.cacheHits! + optimizedWarmResult.cacheMisses!) * 100).toFixed(1)}%`);
  ...
  const complexImprovement = ((enhancedComplexResult.duration - optimizedComplexResult.duration) / enhancedComplexResult.duration * 100).toFixed(1);
