...
async function backfillHandles() {
  ...
  try {
    ...
    while (true) {
      ...
      if (batchResolved === 0) {
        consecutiveFailures++;
        console.log(`  ⚠ No progress in batch (failure ${consecutiveFailures}/${MAX_CONSECUTIVE_FAILURES})`);
        
        if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
          console.log("\n✗ Too many consecutive failures. Stopping backfill.");
          console.log("  This may indicate a PLC directory outage or network issues.");
          console.log("  Run this script again later to retry.");
          break;
        }
        
        // Exponential backoff on repeated failures
        await new Promise(resolve => setTimeout(resolve, RATE_LIMIT_MS * Math.pow(2, consecutiveFailures)));
      } else {
        consecutiveFailures = 0;
      }

      console.log(`  Progress: ${totalResolved + totalFailed}/${totalUnresolved} (${totalResolved} resolved, ${totalFailed} failed)`);

      await new Promise(resolve => setTimeout(resolve, RATE_LIMIT_MS));

      if (totalResolved + totalFailed >= 1000) {
        console.log("\n⚠ Processed 1,000 users. Stopping to avoid rate limits.");
        console.log("  Run this script again to continue resolving more handles.");
        break;
      }
    }
    ...
    console.log(`  - Remaining: ${totalUnresolved - totalResolved - totalFailed} users`);
    console.log("=".repeat(60));
