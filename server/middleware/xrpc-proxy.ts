    ...
    if (authPayload.sessionId) {
      ...
      if (!session) {
        return res.status(401).json({
          ...
          message: 'Your session could not be found or has expired.',
        });
      }
      ...
      if (!pdsAccessToken || !pdsEndpoint) {
        console.error(
          `[XRPC_PROXY] Session for ${session.userDid} is missing PDS credentials.`
        );
        return res.status(500).json({
          error: 'InvalidSessionState',
          message: 'Session is missing PDS credentials.',
        });
      }
      ...
      if (!isContentTypeSafe(contentType)) {
        console.error(
          `[XRPC_PROXY] Unsafe content-type from PDS: ${contentType}`
        );
        return res.status(500).json({
          error: 'UnsafeResponse',
          message: 'PDS returned unsafe content type.',
        });
      }
      ...
      if (!pdsEndpoint) {
        console.error(
          `[XRPC_PROXY] Could not resolve PDS endpoint for DID: ${userDid}`
        );
        return res.status(500).json({
          error: 'PDSNotFound',
          message: 'Could not determine PDS endpoint for user.',
        });
      }
